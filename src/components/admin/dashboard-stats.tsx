...
export function DashboardStats() {
  ...
  const todaysOrders = completed.filter((order) => localDateKey(new Date(order.completedAt)) === today);
  const todayRevenue = todaysOrders.reduce((sum, order) => sum + order.total, 0);
  const currency: Currency = todaysOrders[0]?.currency || completed[0]?.currency || data?.general.defaultCurrency || "IQD";
