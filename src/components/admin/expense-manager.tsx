...
import { CalendarDays, CircleOff, ListOrdered, PlusCircle, ReceiptText, Search, Scale, Tags, Trash2, TrendingDown, TrendingUp, UserRound, Wallet } from "lucide-react";
...
export function ExpenseManager() {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order) => matchesPeriod(localDateKey(order.completedAt), mode, day, month));
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      average: filteredExpenses.length ? Math.round(totalExpenses / filteredExpenses.length) : 0,
      largest
    };
  }, [filteredExpenses, filteredOrders]);

  const categoryTotals = useMemo(() => groupedTotals(filteredExpenses, (expense) => expense.category), [filteredExpenses]);
  const byWhoTotals = useMemo(() => groupedTotals(filteredExpenses, (expense) => expense.byWho), [filteredExpenses]);
                  ...
                  {filteredExpenses.map((expense) => (
                    <tr key={expense.id} className="border-b last:border-0">
                      ...
                      <td className="max-w-[260px] px-3 py-3">
                        <p dir={textDir} className="truncate font-medium">{expense.title}</p>
                      ...
                      <td className="py-3 ps-3">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <Button type="button" variant="outline" size="icon" aria-label={text.delete} title={text.delete} disabled={saving} onClick={() => handleDelete(expense)}>
